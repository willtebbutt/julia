...
function is_effect_free_infer(f::IntrinsicFunction)
...
function intrinsic_effects(f::IntrinsicFunction, argtypes::Vector{Any})
    ...
    effect_free = is_effect_free_infer(f) ? ALWAYS_TRUE : ALWAYS_FALSE
    if (is_pure_intrinsic_infer(f) && !contains_is(_INCONSISTENT_INTRINSICS, f)) || f === Intrinsics.pointerset || f === Intrinsics.atomic_pointerset || f === Intrinsics.atomic_fence
        consistent = ALWAYS_TRUE
    else
        consistent = ALWAYS_FALSE
    end
    nothrow = intrinsic_nothrow(f, argtypes)
    inaccessiblememonly = effect_free === ALWAYS_TRUE && !(f === Intrinsics.pointerref) ? ALWAYS_TRUE : ALWAYS_FALSE
